...
YUI.add('yui-chosen', function (Y) {
  ...
  var ClassNamePrefix = 'chosen'; //chosen
  ...
  Y.extend(YUIChosen, AbstractChosen, {
    setup: function() {
      ...
      return this.is_rtl = this.form_field_yui.hasClass("chosen-rtl");
    },
    finish_setup: function() {
      return this.form_field_yui.addClass('chosen-done');
    },
    set_up_html: function() {
      ...
      if (this.is_multiple) {
        this.container.setContent('<ul class="chosen-choices"><li class="search-field"><input type="text" value="" placeholder="' + this.default_text + '" class="default" autocomplete="off" style="width:25px;" /></li></ul><div class="chosen-drop"><ul class="chosen-results"></ul></div>');
      } else {
        this.container.setContent('<a href="javascript:void(0)" class="chosen-single chosen-default" tabindex="-1"><span>' + this.default_text + '</span><div><b></b></div></a><div class="chosen-drop"><div class="chosen-search"><input type="text" autocomplete="off" placeholder="search" /></div><ul class="chosen-results"></ul></div>');
      }
      this.form_field_yui.hide().insert(this.container, 'after');
      this.dropdown = this.container.one('div.chosen-drop');
      this.search_field = this.container.one('input');
      this.search_results = this.container.one('ul.chosen-results');
      this.search_field_scale();
      this.search_no_results = this.container.one('li.no-results');
      if (this.is_multiple) {
        this.search_choices = this.container.one('ul.chosen-choices');
        this.search_container = this.container.one('li.search-field');
      } else {
        this.search_container = this.container.one('div.chosen-search');
        this.selected_item = this.container.one('.chosen-single');
      }
      this.results_build();
      this.set_tab_index();
      ...
      if (!this.is_disabled) {
        ...
        if (!(e != null && e.target.hasClass("search-choice-close"))) {
          if (!this.active_field) {
            ...
            this.results_show();
          } else if (!this.is_multiple && e && ((e.target === this.selected_item) || e.target.ancestor("a.chosen-single"))) {
            e.preventDefault();
            this.results_toggle();
          }
